...
# Muestra el tablero con el que se juega - VERSIÓN CORREGIDA
def tablero_base(tablero_mostrar):
    print("    ", end="")

    # Imprimir letras de columnas - 3 espacios entre ellas
    ...
    for i in range(columna):
        ...
        for j in range(columna):
            elemento = str(tablero_mostrar[i][j])
            # Asegurar que cada elemento ocupe 3 caracteres
            if elemento == "⬛️":
                print(elemento + " ", end="")  # ⬛️ + espacio = 3 caracteres
            elif len(elemento) == 1:
                print(" " + elemento + " ", end="")  # espacio + número + espacio = 3 caracteres
            elif len(elemento) == 2:
                print(elemento + " ", end="")  # número + espacio = 3 caracteres
            else:
                print(elemento + " ", end="")  # otros casos
        print()


# Crea el trablero oculto por primera vez
def crear_tablero_cuadrados(tablero_mostrar, columna):
    # Limpiar el tablero antes de crear uno nuevo
    tablero_mostrar.clear()
    for i in range(columna):
...
# Muestra el tablero final con todas las minas como explosiones - VERSIÓN CORREGIDA
def mostrar_tablero_con_explosion(tablero, posicion_fila, posicion_columna):
    print("\nTablero completo:")
    print("    ", end="")

    # Imprimir letras de columnas - MISMO FORMATO que tablero_base()
    for i in range(columna):
        print(chr(65 + i), end="  ")
    print()
    ...
    for i in range(columna):
        ...
        # Mostrar la fila i de la matriz tablero
        for j in range(columna):
            elemento = str(tablero[i][j])
            # Si es la mina que hizo explotar, mostrar explosión
            if i == posicion_fila and j == posicion_columna and elemento == "*":
                print("💥 ", end="")  # Explosión (2 caracteres + espacio = 3 caracteres)
            # Si es otra mina, mostrar bomba
            elif elemento == "*":
                print("💣 ", end="")  # Bomba (2 caracteres + espacio = 3 caracteres)
            # Mostrar los ceros
            elif elemento == "0":
                print(" 0 ", end="")  # espacio + 0 + espacio = 3 caracteres
            # Para números de 1 dígito (1-8)
            elif len(elemento) == 1 and elemento != "*":
                print(" " + elemento + " ", end="")  # espacio + número + espacio = 3 caracteres
            # Para números de 2 dígitos (solo ocurre en tableros muy grandes)
            elif len(elemento) == 2:
                print(elemento + " ", end="")  # número + espacio = 3 caracteres
            # Para cualquier otro caso
            else:
                print("   ", end="")  # 3 espacios vacíos
        print()
...
def comprobar(indice_fila, indice_columna, tablero, tablero_mostrar, tablero_victoria):
    ...
    if tablero[indice_fila][indice_columna] == "*":
        ...
        # Mostrar el tablero completo con explosión en la mina que hizo perder
        mostrar_tablero_con_explosion(tablero, indice_fila, indice_columna)
        return False
    else:
        ...
        if victoria == tablero_victoria:
            ...
            # Mostrar el tablero completo cuando ganas
            mostrar_tablero_con_explosion(tablero, -1, -1)  # -1 para indicar que no hubo explosión
            return False
